...
class GameIntegration {
    ...
    spawnAITanks(gameMode) {
        ...
        if (this.systems.mapRenderer && this.systems.mapRenderer.isMapLoaded()) {
            ...
            // Fallback to random spawn points
            for (let i = 0; i < aiCount; i++) {
                spawnPoints.push(this.getRandomSpawnPoint());
            }
        }

        for (let i = 0; i < aiCount; i++) {
            const personality = personalities[i % personalities.length];
            const spawnPoint = spawnPoints[i] || this.getRandomSpawnPoint();

            this.systems.ai.addAITank(spawnPoint.x, spawnPoint.y, personality, 'ai');
        }

        console.log(`🤖 Spawned ${aiCount} AI tanks on map`);
    }
...
window.gameEvents = {
    dispatch: (event, data) => {
        if (window.GameIntegration && typeof window.GameIntegration.dispatchEvent === 'function') {
            ...
            console.warn(`GameIntegration.dispatchEvent not available for event: ${event}`);
        }
    },
