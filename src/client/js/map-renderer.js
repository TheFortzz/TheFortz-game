...
class MapRenderer {
    ...
    async preloadAssets() {
        ...
        const groundTextures = [
            ...
            'ground7', 'ground8', 'ground9', 'ground10', 'ground11', 'ground12',
            'ground13', 'ground14', 'ground15', 'ground16', 'ground17', 'ground18'
        ];

        const groundPromises = groundTextures.map(async (groundType, index) => {
            try {
                const img = new Image();
                const filename = index === 0 ? '_Group_.png' : `_Group_ (${index}).png`;
                ...
                this.groundImages.set(groundType, img);
            } catch (error) {
                console.warn(`Failed to load ground texture ${groundType}:`, error);
                // Create fallback colored rectangle
                this.groundImages.set(groundType, this.createFallbackGroundTexture(groundType));
            }
        });
        ...
        const buildingPromises = buildingTypes.map(async (buildingType) => {
            try {
                ...
                const possiblePaths = [
                    `/assets/Buildings/${buildingPathMap[buildingType]}.png`,
                    `/assets/Buildings/${buildingType}.png`,
                    `/assets/Buildings/${buildingType}/${buildingType}.png`,
    ...
    render(ctx, camera) {
        if (!this.isLoaded || !this.currentMap) {
            this.renderDefaultTerrain(ctx, camera);
            return;
        }
    ...
    renderDefaultTerrain(ctx, camera) {
        ...
        const endX = Math.ceil((camera.x + window.innerWidth + 100) / tileSize);
        const startY = Math.floor((camera.y - 100) / tileHeight);
        const endY = Math.ceil((camera.y + window.innerHeight + 100) / tileHeight);
    ...
    renderGroundTiles(ctx, camera) {
        ...
        // Only render tiles visible in the viewport (with small margin)
        const margin = 100;
        const viewLeft = camera.x - margin;
        const viewRight = camera.x + canvasWidth + margin;
        const viewTop = camera.y - margin;
        const viewBottom = camera.y + canvasHeight + margin;
        ...
        for (let row = startRow; row <= endRow; row++) {
            for (let col = startCol; col <= endCol; col++) {
                ...
                if (customTile && this.groundImages.has(customTile.type)) {
                    // Render custom ground tile
                    const groundImg = this.groundImages.get(customTile.type);
                    ctx.drawImage(groundImg, screenX, screenY, tileSize, tileSize * 0.6);
                    tilesRendered++;
                } else {
                    // Render default ground
                    ctx.save();
                    ctx.translate(screenX, screenY);
    ...
    renderBuildings(ctx, camera) {
        ...
        // Only render buildings visible in viewport
        const visibleBuildings = this.buildings
            .filter(building => {
                ...
                return screenX > -building.width && screenX < canvasWidth + building.width &&
                       screenY > -building.height && screenY < canvasHeight + building.height;
            })
            .sort((a, b) => a.y - b.y);
        ...
        visibleBuildings.forEach(building => {
            ...
            if (building.image) {
                ...
                ctx.beginPath();
                ctx.ellipse(screenX, screenY + building.height * 0.8,
