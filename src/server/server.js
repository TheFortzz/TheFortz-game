...
// Serve index.css from client root for backward compatibility
...
// Server only tracks player positions and basic state
// No rendering logic on server
...
function generateWalls(count = 80, gameWidth = 7500, gameHeight = 7500) {
    const walls = [];
    const hexSize = 60; // Match the ground hexagon size EXACTLY (was 40, now 60)
    ...
    const maxRow = Math.floor((gameHeight - hexSize) / vertSpacing);
    const maxCol = Math.floor((gameWidth - hexSize) / hexWidth);

    // Store used positions to avoid duplicates
    const usedPositions = new Set();

    let attempts = 0;
    while (walls.length < count && attempts < count * 10) {
        attempts++;

        // Random row and column within valid grid
        const row = Math.floor(Math.random() * maxRow);
        const col = Math.floor(Math.random() * maxCol);

        // Create unique position key
        const posKey = `${row}_${col}`;
        if (usedPositions.has(posKey)) continue;

        // Calculate hexagon center position EXACTLY as client does
        const y = row * vertSpacing;
        const offsetX = (row % 2 === 0) ? 0 : hexWidth / 2;
        const x = col * hexWidth + offsetX;

        // Only create wall if STRICTLY within game boundaries (matching client logic)
        if (x >= hexSize && x <= gameWidth - hexSize &&
            y >= hexSize && y <= gameHeight - hexSize) {
            walls.push({
                id: `wall_${walls.length}`,
                x: x, // Center X of hexagon
                y: y, // Center Y of hexagon
                radius: hexSize, // Hexagon radius for collision (now 60)
                width: hexSize * 2, // For compatibility
                height: hexSize * 2, // For compatibility
                color: '#666'
            });
...
Object.keys(gameModes).forEach(mode => {
    ...
    gameModes[mode].powerUps = []; // Power-ups removed
});
...
wss.on('connection', (ws, req) => {
    ...
    if (!isLobby) {
        ...
        gameState.players[playerId] = {
            id: playerId,
            x: 0, // Exact center of map
            y: 0, // Exact center of map
            angle: 0,
            tankDirection: 0,
